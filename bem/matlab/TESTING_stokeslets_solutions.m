...
eta = 1;
a = 0.5;
dip = 90; % atan2d(Δy,Δx) where Δy=y2-y1,Δx=x2-x1
...
% testing stress by finite differences
[duxdx,duxdy] = gradient(reshape(ux(:,2),ny,nx),xvec,yvec);
[duydx,duydy] = gradient(reshape(uy(:,2),ny,nx),xvec,yvec);
figure(20),clf
subplot(3,1,1)
toplot=reshape((duxdy+duydx)./2,ny,nx);
pcolor(xvec,yvec,toplot), shading interp, hold on
contour(xvec,yvec,toplot,10,'k-')
axis tight equal
colorbar
title('\sigma_{xy} (testing)')
subplot(3,1,2)
toplot=reshape(duxdx,ny,nx);
pcolor(xvec,yvec,toplot), shading interp, hold on
contour(xvec,yvec,toplot,10,'k-')
axis tight equal
colorbar
title('\sigma_{xx} (testing)')
clim([-1,1]*0.5)
subplot(3,1,3)
toplot=reshape(duydy,ny,nx);
pcolor(xvec,yvec,toplot), shading interp, hold on
contour(xvec,yvec,toplot,10,'k-')
axis tight equal
colorbar
title('\sigma_{yy} (testing)')
clim([-1,1]*0.5)
colormap(turbo(100))
...
% plot line plots along y-transect
xpos = 1e-3;
